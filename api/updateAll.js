...
// Helper to log messages to console
  ...
  try {
    ...
    const updatedResults = {
      ...
      results: games.map(game => {
        ...
        const winner = !isNaN(homeScore) && !isNaN(awayScore)
          ? (homeScore > awayScore ? homeTeam.team.displayName : awayTeam.team.displayName)
          : '';
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      for (const [weekStr, weekPicks] of Object.entries(picks)) {
        ...
        const weekResults = weekNum === weekNumber ? updatedResults : (await db.collection('results').doc(`week${weekNum}`).get()).data();
